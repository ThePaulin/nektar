...
export function getFfmpegPathCandidates({ platform, arch, cwd, resourcesPath, pathEnv, }) {
...
async function writeStreamToFile(url, filePath) {
...
async function materializeAssets(request, workspaceDir, handlers, jobId) {
...
function detectFfmpegEncoders(ffmpegPath) {
...
export function buildFfmpegCommand({ request, materializedAssets, outputPath, availableEncoders, }) {
    const inputArgs = [];
    const filterParts = [];
    ...
    for (const clip of clips) {
        ...
        if (!clip.assetRef && clip.type !== 'text' && clip.type !== 'subtitle')
            continue;
        if (clip.type === 'text' || clip.type === 'subtitle') {
            ...
            filterParts.push(`[${sourceLabel}]drawtext=text='${ffmpegEscapeText(clip.content || '')}':fontsize=${fontSize}:fontcolor=${color}:x=${textX}:y=${textY}:enable='${enable}'[vout${layerIndex}]`);
            layerIndex += 1;
            continue;
        }
        ...
        if (asset.kind === 'audio' && (track?.isMuted || volume <= 0))
            continue;
        if (asset.kind !== 'image') {
        ...
        if (asset.kind !== 'image' && assetHasAudioStream(request, asset)) {
            if (track?.isMuted || volume <= 0)
                continue;
            filterParts.push(`[${currentInput}:a]atrim=duration=${formatFfmpegSeconds(visibleDuration)},asetpts=PTS-STARTPTS,volume=${volume},adelay=${offset}|${offset}[a${clip.id}]`);
...
function parseFfmpegProgress(chunk, request) {
...
export async function startDesktopExport(request, handlers) {
    ...
    if (!existsSync(ffmpegPath)) {
        throw new Error(`FFmpeg was not found. Checked: ${ffmpegCandidates.join(', ')}. Add platform binaries under resources/ffmpeg/<platform>-<arch>/ or install ffmpeg on this machine.`);
    }
    ...
    const emitProgress = (progress) => handlers.emitProgress({ jobId, ...progress });
...
export async function getDesktopExportResult(jobId) {
...
export async function cancelDesktopExport(jobId) {
    const job = jobRegistry.get(jobId);
    if (!job)
        return;
...
export async function copyDesktopExportResult(jobId, targetPath) {
